...
int valid(int row, int column){
	if(aRecordBoard[row][column]==1||aRecordBoard[row][column]==3||aRecordBoard[row][column]==2||aRecordBoard[row][column]==4)
		return 0;
...
int main(){
	...
	while(counter++){
		...
		mygetline(input, MAX);
		if(input[0]=='q'&&input[1]=='u'&&input[2]=='i'&&input[3]=='t'&&input[4]=='\n'&&input[5]=='\0'){
			printf("Bye.\n");	
			break;
		}

		if(isdigit(input[0])&&isdigit(input[1])&&isalpha(input[2])&&input[3]=='\n'&&input[4]=='\0'){
			row=15-(input[0]-'0')*10-(input[1]-'0');
			column=input[2]-'a';
		}
		else if(isdigit(input[0])&&isalpha(input[1])&&input[2]=='\n'&&input[3]=='\0'){
			row=15-(input[0]-'0');
			column=input[1]-'a';
		}
		else{
		...
		//aRecordBoard[row][column]=(counter%2)?3:4;
		for(int i=0; i<SIZE; ++i)
			for(int j=0; j<SIZE; ++j){
				if(aRecordBoard[i][j]==3)
					aRecordBoard[i][j]=1;
				if(aRecordBoard[i][j]==4)
					aRecordBoard[i][j]=2;
			}
